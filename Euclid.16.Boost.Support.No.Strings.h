...
namespace Euclid_Prover {
	...
	*/
	...
	inline BigInt512_t ComputeHash(const std::vector<std::string>& Theorem_Src)
	{
        ...
        for (const std::string& z : Theorem_Src)
        {
			...
			retbuff *= SymbolToPrime_UInt64MultiMap[z];
        }    
...
{ { "123", "+", "123" }, { "246" } }
		...
		auto PopulateLocalHashLibraryUInt64Vec = [&] (const PROOF_STEP& InLocalProofStep3DStdStrVec) -> HASH
		{
			...
			{
				const BigInt512_t LHSPrimeProductUInt64 = ComputeHash(Axiom_i[LHS]);
				const BigInt512_t RHSPrimeProductUInt64 = ComputeHash(Axiom_i[RHS]);
				Hash3DUInt64Vec.emplace_back(std::array<BigInt512_t,2>{ LHSPrimeProductUInt64, RHSPrimeProductUInt64 });
			}
			__stdtraceout__("PopulateProofStepLibrary3DUInt64Vec");
			return Hash3DUInt64Vec;
		};

		const HASH LocalHashLibrary3DUInt64Vec = 
        ...
        */
		...
		auto BuildCallGraph = [&] ()
		{
			const BigInt512_t SIZE = LocalHashLibrary3DUInt64Vec.size();
			for (BigInt512_t i = 0; i < SIZE; ++i)
			{
				for (BigInt512_t j = 0; j < SIZE; ++j)
				{
					...
					(
						ii != jj && (
						LocalHashLibrary3DUInt64Vec[ii][LHS] % LocalHashLibrary3DUInt64Vec[jj][LHS] ||
						LocalHashLibrary3DUInt64Vec[ii][LHS] % LocalHashLibrary3DUInt64Vec[jj][RHS] ||
						LocalHashLibrary3DUInt64Vec[ii][RHS] % LocalHashLibrary3DUInt64Vec[jj][LHS] ||
						LocalHashLibrary3DUInt64Vec[ii][RHS] % LocalHashLibrary3DUInt64Vec[jj][RHS] )
					)
					{
        ...
        std::unordered_map<BigInt512_t, PROOF_STEP>
		...
		Proof.LHSPrimaryKeyUInt64 = LocalHashLibrary3DUInt64Vec[0][LHS];
		Proof.RHSPrimaryKeyUInt64 = LocalHashLibrary3DUInt64Vec[0][RHS];
        ...
        while (!Tasks_Thread.empty() && !QED)
        {
			...
			BigInt512_t LHSPrimaryKeyUInt64 = TempProofStruct.LHSPrimaryKeyUInt64;
			BigInt512_t RHSPrimaryKeyUInt64 = TempProofStruct.RHSPrimaryKeyUInt64;
            ...
            for(BigInt512_t i = 1; i < I; ++i)
            {
				...
				const BigInt512_t LHSAxiomPrimaryKeyUInt64 = LocalHashLibrary3DUInt64Vec[ii][LHS];
				const BigInt512_t RHSAxiomPrimaryKeyUInt64 = LocalHashLibrary3DUInt64Vec[ii][RHS];
				...
				auto LHSReduceFastForwardFound = [&] () -> bool {
					...
					if (LHSRouteHistoryMap.find(UpdateLHSPrimaryKeyUInt64) == LHSRouteHistoryMap.end())
						LHSRouteHistoryMap.emplace(UpdateLHSPrimaryKeyUInt64, TempProofStep3DStdStrVec_0000);
					...
					if (RHSRouteHistoryMap.find(UpdateLHSPrimaryKeyUInt64) != RHSRouteHistoryMap.end()) {
						std::cout << "Proof found in Module_000" << << " via Fast-Forward (FF)" << std::endl;
						__stdlog__({ "Proof found in Module_000",," via Fast-Forward (FF)" });
						auto iProofStep2DStdStrVec = RHSRouteHistoryMap[UpdateLHSPrimaryKeyUInt64].begin();
						const auto iProofStepEND = RHSRouteHistoryMap[UpdateLHSPrimaryKeyUInt64].end();
				...
				auto LHSExpandFastForwardFound = [&] () -> bool {
					CallGraphMap[last_GUID][ii] = true;
					...
					if (LHSRouteHistoryMap.find(UpdateLHSPrimaryKeyUInt64) == LHSRouteHistoryMap.end())
						LHSRouteHistoryMap.emplace(UpdateLHSPrimaryKeyUInt64, TempProofStep3DStdStrVec_0001);

					PROOF_STRUCT TempProofStruct_0000;
					TempProofStruct_0000._ = TempProofStep3DStdStrVec_0001;
					TempProofStruct_0000.LHSPrimaryKeyUInt64 = UpdateLHSPrimaryKeyUInt64;
					TempProofStruct_0000.RHSPrimaryKeyUInt64 = TempProofStruct.RHSPrimaryKeyUInt64;
					TempProofStruct_0000.last_guid = ii;
				...
				if (lhs_reduce_flag && LHSReduceFastForwardFound())
					break;

				if (lhs_expand_flag && LHSExpandFastForwardFound())
					break;
